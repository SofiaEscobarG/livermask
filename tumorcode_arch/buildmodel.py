...
def DepthwiseConvBlock(model_in):
    ...
    if settings.options.regularizer:
        if settings.options.D3:
            model = DepthwiseConv3D(kernel_size=(3,3,3), padding='same', depth_multiplier=_dm, activation=settings.options.activation, depthwise_regularizer=l1(settings.options.regurizer))(model_in)
        else:
            model = DepthwiseConv2D(kernel_size=(3,3), padding='same', depth_multiplier=_dm, activation=settings.options.activation, depthwise_regularizer=l1(settings.options.regurizer))(model_in)
    else:
        if settings.options.D3:
            model = DepthwiseConv3D(kernel_size=(3,3,3), padding='same', depth_multiplier=_dm, activation=settings.options.activation)(model_in)
        else:
            model = DepthwiseConv2D(kernel_size=(3,3), padding='same', depth_multiplier=_dm, activation=settings.options.activation)(model_in)
