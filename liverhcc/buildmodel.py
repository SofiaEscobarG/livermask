import numpy as np
import keras
from keras.layers import Input, Conv2D, Conv3D, UpSampling2D, UpSampling3D, Lambda, SpatialDropout2D, SpatialDropout3D, Dense, Layer, Activation, BatchNormalization, AveragePooling2D, AveragePooling3D, concatenate, Add, LocallyConnected2D, DepthwiseConv2D
    ...
    if settings.options.gpu > 1:
        with tf.device('/cpu:0'):
            if settings.options.D3: 
                ...
                layer_in = Input(shape=(settings.options.thickness, settings.ny, settings.nx))
            else: 
